...
int main()
{
    ...
    catch(RationalException re) { exceptionError(); }
...
void doOneTest(int num1, int denom1, int num2, int denom2)
{
    ...
    try
    {
        ...
        if(expectingException) noExceptionError("/", opd2, opd2.getNumerator(), opd2.getDenominator());
...
{
    ostringstream oss;
    if(r.getDenominator() != 1) oss << r.getNumerator() << "/" << r.getDenominator();
    else oss << r.getNumerator();
    return oss.str();
}
