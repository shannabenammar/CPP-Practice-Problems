 ...
 * - A function named `fullUp` that accepts no parameters and returns a
 * double. `fillUp` increase `amount` to `capacity` and returns the difference
 * between the value of `capacity` and the original value of `amount` (that is,
 * the amount of gas that is needed to fill the tank to capacity).
 */
